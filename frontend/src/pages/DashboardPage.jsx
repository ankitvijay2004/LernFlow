...
import { Book, Play, Award, Users, ChevronRight, Flame, Target, ChartNoAxesCombined } from 'lucide-react';
    ...
    return (
        <div className="container section-padding animate-fade">
            <header className="dashboard-header">
                <div>
                    <p className="home-badge"><ChartNoAxesCombined size={15} /> Productivity dashboard</p>
                    <h1 style={{ fontSize: 'clamp(2rem, 4.2vw, 3.5rem)', marginBottom: '0.5rem', fontWeight: 800, letterSpacing: '-0.03em' }}>
                        Welcome back, <span className="gradient-text">{user?.name}</span>
                    </h1>
                    <p style={{ color: 'var(--text-muted)', fontSize: '1.06rem' }}>
                        You have <span style={{ color: 'var(--secondary)', fontWeight: 800 }}>{enrollments.length}</span> courses currently in progress.
                    </p>
                </div>

                <div className="dashboard-quick-stats">
                    <div className="glass quick-stat-card">
                        <Flame size={18} />
                        <div>
                            <strong>12h</strong>
                            <p>Weekly Focus</p>
            ...
            {loading ? (
                ...
                <section>
                    ...
                    {enrollments.length > 0 ? (
                        <div className="resp-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.4rem' }}>
                            {enrollments.map(({ course, completedLessons }, idx) => (
                                <div key={course._id} className={`glass card-hover animate-fade stagger-${(idx % 3) + 1}`} style={{ overflow: 'hidden', borderRadius: '16px' }}>
                                    ...
                                    <div style={{ padding: '1.3rem 1.15rem' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '0.5rem' }}>
                                            <h3 style={{ fontSize: '1.15rem', fontWeight: 700, lineHeight: 1.3 }}>{course.title}</h3>
                                        </div>
                                        ...
                                        <div style={{ marginBottom: '1.5rem' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.75rem', fontWeight: 600 }}>
                                                ...
                                                <span style={{ color: 'var(--text-main)' }}>
                                                    {Math.round(((completedLessons?.length || 0) / (course.lessons?.length || 1)) * 100) || 0}%
                                                </span>
                                            </div>
                                            <div style={{ background: 'var(--glass-border)', height: '10px', borderRadius: '999px', overflow: 'hidden' }}>
                                                <div style={{
                                                    background: 'linear-gradient(90deg, var(--primary), var(--secondary))',
                                                    height: '100%',
                                                    borderRadius: '999px',
                                                    width: `${((completedLessons?.length || 0) / (course.lessons?.length || 1)) * 100 || 0}%`,
                                                    transition: 'width 1s cubic-bezier(0.4, 0, 0.2, 1)'
                                                }}></div>
                                            </div>
                                        </div>

                                        <Link to={`/course/${course._id}/learn`} className="btn-primary" style={{ width: '100%', padding: '0.75rem' }}>
            ...
            <section style={{ marginTop: '3rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                    <div className="glass" style={{ padding: '0.9rem 1.2rem', display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
                        <Award size={24} color="#fbbf24" />
                        <span>Quick Learner</span>
                    </div>
                    <div className="glass" style={{ padding: '0.9rem 1.2rem', display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
                        <Book size={22} color="var(--secondary)" />
